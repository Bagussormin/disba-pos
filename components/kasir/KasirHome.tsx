...
      // 🔥 PERBAIKAN 1: Membaca dari tabel 'menus' dan memakai 'menu_id'
...
      .from("order_items")
      .select(`*, menus(name)`) // 🔥 PERBAIKAN 2: Menggunakan relasi tabel 'menus'
      .eq("bill_id", billId)
...
        id: item.menu_id, // 🔥 KRUSIAL: Membawa ID agar Robot Potong Stok Gudang bisa bekerja!
...
  // 🔥 FUNGSI PEMBAYARAN & CETAK TERBARU
...
    setLoading(true);
    try {
      // 1. 🔥 BUAT NOMOR TRANSAKSI KEREN (INV/NES/YYMMDD/001)
      const today = new Date();
      const dateStr = today.toISOString().slice(2, 10).replace(/-/g, ""); // Hasil format: 260315
      
...
      // 2. 🔥 AMBIL PENGATURAN STRUK DARI DATABASE
...
        pb1: getTax(),
        total: getGrandTotal(),
        items: orderItems, // Robot pemotong stok membaca JSON ini (ada menu_id & qty)
...
      // 4. 🔥 SIAPKAN DATA UNTUK DIKIRIM KE PRINTER
      const receiptData = {
        orderId: receiptNo,
...
        change: getChange(),
        
        // Data Tambahan dari Receipt Builder
