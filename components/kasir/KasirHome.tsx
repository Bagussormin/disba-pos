...
export default function KasirHome() {
  ...
  const handleStartShift = async () => {
    const { data, error } = await supabase.from("shifts").insert({ tenant_id: tenantId, cashier_name: localStorage.getItem("username") || "KASIR", starting_cash: Number(startCash), status: 'open', start_time: new Date().toISOString() }).select().single();
    ...
    <div className="fixed inset-0 bg-[#020617] text-white p-1 uppercase italic font-sans flex flex-col overflow-hidden">
      ...
      <header className="flex justify-between items-center bg-black/60 border-b border-white/5 p-2 mb-1 shadow-2xl">
        ...
        <div className="flex gap-1.5">
          <button onClick={() => setShowItemReportModal(true)} className="h-7 px-3 bg-blue-600/10 rounded-md border border-blue-500/20 text-[8px] font-black active:scale-95">REKAP</button>
          <button onClick={() => setShowCloseShiftModal(true)} className="h-7 px-3 bg-orange-600/10 rounded-md border border-orange-500/20 text-[8px] font-black active:scale-95">SHIFT</button>
      ...
      <div className="flex-1 flex gap-1 min-h-0 overflow-hidden">
        ...
        {/* KOLOM 1: MEJA (200px) */}
        <div className="w-52 bg-black/20 rounded-xl border border-white/5 p-2 overflow-y-auto no-scrollbar">
          {Array.from(new Set(tables.map(t => (t.area || "REGULER").toUpperCase()))).map(area => (
            <div key={area} className="mb-5">
              <p className="text-[8px] font-black text-gray-700 mb-2 tracking-[0.2em] text-center border-b border-white/5 pb-1">{area}</p>
                ...
                {tables.filter(t => (t.area || "REGULER").toUpperCase() === area).map(t => {
                   ...
                   return (
                    <button key={t.id} onClick={() => setSelectedTable(t)}
                      className={`h-14 rounded-xl border-2 transition-all text-[11px] font-black flex flex-col items-center justify-center ${
          ...
          {currentOrder ? (
            <div className="flex flex-col h-full">
              <div className="p-3 border-b border-white/5 bg-white/[0.02] flex justify-between items-center">
                ...
                <button onClick={() => setSelectedTable(null)} className="text-gray-600 hover:text-white transition-all"><X size={20}/></button>
              </div>

              {/* AREA LIST PESANAN - SEKARANG SANGAT LUAS */}
            ...
            <div className="flex-1 flex flex-col items-center justify-center opacity-5 grayscale">
              ...
              <p className="text-sm font-black mt-6 tracking-[1em] uppercase">Ready_Station</p>
            </div>
          )}
        </div>

        {/* KOLOM 3: PAYMENT SIDEBAR (340px) */}
        <div className="w-[340px] bg-black/60 rounded-xl border border-white/5 flex flex-col overflow-hidden p-4 shadow-2xl backdrop-blur-3xl">
          {currentOrder ? (
            <div className="flex flex-col h-full">
              <h3 className="text-[10px] font-black text-gray-600 tracking-[0.3em] mb-6 border-b border-white/10 pb-3 flex items-center gap-2"><CreditCard size={14}/> CHECKOUT_PANEL</h3>
              
              <div className="space-y-3 mb-6 bg-white/[0.02] p-4 rounded-2xl border border-white/5 shadow-inner">
                ...
                <div className="flex justify-between items-center text-[11px] font-black text-blue-500 italic">
                    <span>DISCOUNT</span>
                    <input type="number" className="w-24 bg-blue-500/10 border border-blue-500/30 rounded-lg px-2 py-1 text-right text-blue-400 outline-none font-black" value={discount || ""} onChange={(e) => setDiscount(Number(e.target.value))} />
                </div>
              ...
              <div className="mt-auto border-t-2 border-dashed border-white/10 pt-6">
                <p className="text-[10px] font-black text-blue-500 italic tracking-widest mb-1">TOTAL_BILLING_DUE</p>
                <p className="text-5xl font-black italic tracking-tighter text-white mb-8 border-b-4 border-white pb-3 leading-none shadow-blue-500/20">
                    RP {getGrandTotal().toLocaleString()}
                </p>
                
                <div className="grid grid-cols-2 gap-2 mb-4">
                  <button onClick={() => { setPaymentMethod("CASH"); setSelectedBank(null); }} className={`py-4 rounded-xl text-[11px] font-black border transition-all flex items-center justify-center gap-2 ${paymentMethod === 'CASH' ? 'bg-blue-600 border-blue-400 shadow-xl' : 'bg-white/5 border-white/10 opacity-30'}`}><Banknote size={16}/> CASH</button>
                  <button onClick={() => { setPaymentMethod("TRANSFER"); setPaidAmount(getGrandTotal()); }} className={`py-4 rounded-xl text-[11px] font-black border transition-all flex items-center justify-center gap-2 ${paymentMethod === 'TRANSFER' ? 'bg-purple-600 border-purple-400 shadow-xl' : 'bg-white/5 border-white/10 opacity-30'}`}><CreditCard size={16}/> BANK</button>
                </div>

                {paymentMethod === "TRANSFER" ? (
                  <div className="mb-4 space-y-2 animate-in fade-in slide-in-from-bottom-2">
                    <p className="text-[9px] font-black text-purple-400 italic px-1 uppercase tracking-widest">Tujuan_Rekening:</p>
                    <div className="flex flex-col gap-1.5 max-h-32 overflow-y-auto no-scrollbar">
                        {banks.map(b => (
                            <button key={b.id} onClick={() => setSelectedBank(b)} className={`w-full p-3 rounded-xl border text-left transition-all relative ${selectedBank?.id === b.id ? 'bg-purple-600 border-purple-400 shadow-lg' : 'bg-white/5 border-white/10 opacity-40'}`}>
                                <p className="text-[10px] font-black text-white">{b.bank_name}</p>
                                <p className="text-[12px] font-mono font-black text-white/90">{b.account_number}</p>
                  ...
                  <div className="mb-4">
                    <p className="text-[10px] font-black text-blue-500 italic mb-2 uppercase tracking-widest">Jumlah_Terima_Cash:</p>
                    <input ref={cashInputRef} type="number" className="w-full bg-blue-500/10 border-2 border-blue-500/50 rounded-2xl py-5 px-5 text-4xl font-black text-blue-400 outline-none focus:border-blue-400 transition-all shadow-inner text-center" placeholder="0" value={paidAmount || ""} onChange={(e) => setPaidAmount(Number(e.target.value))} />
                    {paidAmount >= getGrandTotal() && (
                        <div className="mt-4 flex justify-between items-center bg-emerald-500/10 p-4 rounded-2xl border border-emerald-500/30 animate-pulse">
                            <span className="text-[10px] font-black text-emerald-500 italic">KEMBALIAN:</span>
                            <span className="text-2xl font-black text-emerald-400 font-mono">+{getChange().toLocaleString()}</span>
                        </div>
                    )}
                  </div>
                )}
                ...
                <button 
                    ...
                    className={`w-full py-6 rounded-[2rem] font-black text-[13px] flex items-center justify-center gap-3 transition-all shadow-2xl active:scale-95 uppercase tracking-widest ${
                ...
                >
                  {loading ? <Loader2 className="animate-spin" /> : <><Printer size={20}/> Settle_&_Print</>}
                </button>
              </div>
            </div>
          ) : (
            <div className="h-full flex flex-col items-center justify-center text-center px-6">
                <div className="w-16 h-16 bg-white/5 rounded-full flex items-center justify-center mb-6 opacity-20 border border-white/10 shadow-xl">
                    <CheckCircle2 size={32}/>
                </div>
                <p className="text-[10px] font-black text-gray-700 uppercase tracking-[0.3em] leading-loose italic">
                    SILAKAN_PILIH_MEJA<br/>UNTUK_PEMBAYARAN
                </p>
            </div>
          )}
        </div>
      ...
      {/* --- MODALS (REMAINS COMPACT) --- */}
      {showStartShiftModal && (
        <div className="fixed inset-0 bg-[#020617] flex items-center justify-center z-[7000] p-4 backdrop-blur-2xl">
          <div className="text-center p-10 bg-[#0b1120] border border-white/10 rounded-[3rem] w-full max-w-sm shadow-2xl">
            ...
            <h2 className="text-2xl font-black mb-8 uppercase italic tracking-tighter">Buka_Shift_Kasir_</h2>
            <input type="number" autoFocus className="w-full bg-white/5 border border-white/10 rounded-2xl py-6 text-center text-5xl font-black mb-8 outline-none focus:border-blue-500 text-blue-400" placeholder="0" onChange={(e) => setStartCash(Number(e.target.value))} />
          ...
          <div className="bg-white text-black p-8 rounded-[2rem] w-full max-w-[340px] font-mono shadow-2xl relative uppercase italic font-bold">
            ...
            <div className="text-[11px] space-y-1.5">
              ...
              <div className="max-h-40 overflow-y-auto">
                {orderItems.map((item, i) => (<div key={i} className="flex justify-between py-1 border-b border-black/5 last:border-0"><span>{item.qty} {item.name}</span><span>{(item.qty * item.price).toLocaleString()}</span></div>))}
              </div>
              <div className="border-t-4 border-black mt-4 pt-3 flex justify-between font-black text-2xl"><span>TOTAL:</span><span>{getGrandTotal().toLocaleString()}</span></div>
              {paymentMethod === "CASH" && (
            ...
            <div className="flex gap-2 mt-10">
              <button onClick={() => setShowPreviewModal(false)} className="flex-1 py-4 bg-gray-100 rounded-2xl font-black text-[10px] active:scale-95 transition-all">BATAL</button>
              <button onClick={processPayment} disabled={loading} className="flex-[2] py-4 bg-black text-white rounded-2xl font-black text-[10px] flex items-center justify-center gap-2 shadow-xl active:scale-95 transition-all">
                {loading ? <Loader2 className="animate-spin" size={16}/> : <><Printer size={18}/> CONFIRM_&_PRINT</>}
              </button>
            </div>
          </div>
        </div>
